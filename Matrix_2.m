...
for trial_idx = 1:num_trials
    ...
    % Wait for the participant to press a key (no timeout)
...
% Function to wait for key press (no timeout) inside the figure window
function [key, time] = waitforkeypress(fig, t_start)
    ...
    function keyPressCallback(src, event)
        ...
        % Get matrix size from the figure's UserData
